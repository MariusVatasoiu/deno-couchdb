...
import assert from 'https://deno.land/std@0.86.0/node/assert.ts';
import * as querystring from 'https://deno.land/std@0.86.0/node/querystring.ts';

// import Buffer from "https://deno.land/std@0.86.0/node/buffer.ts";
import { ChangesReader } from './changesreader.js';

// import { CookieJar } from "https://raw.githubusercontent.com/ako-deno/cookies/master/mod.ts";
...
const pkg = { name: 'nano', version: '9.0.3' };
...
export default function dbScope (cfg) {
  let serverScope = {}
  ...
  const responseHandler = async function (response, req, opts, resolve, reject, callback) {
    ...
    if (!response.status) {
      ...
      if (reject) {
        reject(new Error(response?.data?.reason || 'error happened in your connection'))
      }
      if (callback) {
        const returnError = {
    ...
    responseHeaders.url = scrubURL(responseHeaders.url)
  ...
  const streamResponseHandler = function (response, req) {
    const statusCode = response.status || 500
    const message = response.statusText
    // console.log('STREAM RESPONSE HANDLER', response, req, response.status);
  ...
  function relax (opts, callback) {
    ...
    const headers = {
      ...
      'user-agent': `${pkg.name}/${pkg.version} (Deno ${Deno.version.deno})`,
    ...
    if (opts.db) {
      req.url = urlResolveFix(req.url, encodeURIComponent(opts.db))
    }
    ...
    if (opts.path) {
      ...
      if (!/^_design|_local/.test(opts.doc)) {
        ...
        req.url += '/' + encodeURIComponent(opts.doc)
      } else {
    ...
    if (opts.body) {
      ...
      if (opts.body instanceof Uint8Array || opts.dontStringify) {
        req.body = opts.body
      } else {
    ...
    if (opts.stream) {
      return fetch(request)
        .then(response => {
          ...
          return streamResponseHandler(response, req);
        })
        .catch(error => {
          // if this is the error thrown by streamResponseHandler
          if(error.statusCode) {
            throw error;
          }
          return streamResponseHandler(error, req);
        });
      ...
      // axios(req)
      //   .then((response) => {
      //     console.log('axios response', response.body);
    ...
    function insertMultipart (doc, attachments, qs, callback) {
      ...
      attachments.forEach(function (att) {
        doc._attachments[att.name] = {
          follows: true,
          length: (att.data instanceof Uint8Array) ? att.data.length : att.data.byteLength,
