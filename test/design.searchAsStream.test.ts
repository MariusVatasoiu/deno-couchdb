...
Deno.test("should be able to access a search index as a stream - POST /db/_design/ddoc/_search/searchname - db.searchAsStream", async () => {
  const response = {
    total_rows: 100000,
    bookmark: 'g123',
    rows: [
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const stream = await db.searchAsStream('ddoc', 'searchname', params);
