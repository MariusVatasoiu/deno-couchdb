...
Deno.test("should be able to access a MapReduce view as a stream - GET /db/_design/ddoc/_view/viewname - db.viewAsStream", async () => {
  const response = {
    rows: [
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const stream = await db.viewAsStream('ddoc', 'viewname');
